                ...
                <v-window-item :value="1">
                    <v-card-text>
                        <v-form ref="form" v-model="valid" lazy-validation>
                            <v-text-field variant="outlined" v-model="nomesistema" :rules="nomesistemaRules"
                                label="Nome" placeholder="Insira o nome do Sistema" required>
                            </v-text-field>

                            <v-row>
                                <v-col>
                                    <v-text-field variant="outlined" v-model="iconsistema" label="Icon"
                                        placeholder="Se houver, insira o icon que o sistema irá ter"
                                        no-data-text="Não cadastrado" required>
                                    </v-text-field>
                                </v-col>
                            </v-row>
                            ...
                            <v-row>
                                <v-col>
                                    <v-text-field variant="outlined" v-model="linksistema" :rules="linksistemaRules"
                                        label="Link" placeholder="Insira o link para o Cadastro do Sistema" required>
                                    </v-text-field>
                                </v-col>
                            </v-row>
                        </v-form>
            ...
            <v-container>
                <v-card-actions variant="tonal">
                    <v-row>
                        ...
                        <v-btn size="large" rounded="lg" variant="tonal" append-icon="mdi-folder-multiple-plus"
                            color="info" elevation="5" v-if="step == 2" text @click="step = 1">
                            Incluir mais Sistemas
                        </v-btn>
                        ...
                        <v-btn rounded="lg" size="large" color="success" :loading="loading" :disabled="!valid"
                            v-if="step == 1" @click="validate(), createSystem(), load()"
...
export default {
    ...
    methods: {
        ...
        async createSystem() {
            ...
            const req = await fetch(`${this.$baseUrlApiMain}/sistema/create`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: dataJson
            });
            await req.json;
            this.messageSuccess = `Sistema ${this.nomesistema} cadastrado com sucesso!`;
        },
    ...
    computed: {
        currentTitle() {
            switch (this.step) {
                ...
                default: return "Cadastro realizado com sucesso!";
            }
        },
    },
}
