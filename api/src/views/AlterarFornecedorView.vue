            ...
            <v-dialog v-model="alertSucces">
                <v-card color="success" max-width="800" min-width="344" height="80">
                    <v-card-text>
                        ...
                        </v-avatar>Fornecedor alterado(a) com sucesso!
                    </v-card-text>
...
export default {
    ...
    methods: {
        ...
        async trazerDadosPai() {
            const req = await fetch(`${this.$baseUrlApiMain}/fornecedor/read/${this.$route.params.id}`);
            const data = await req.json();

            if (data[0].telefone == "Não Informado") {
                this.telefone = '';
            ...
            if (data[0].celular == "Não Informado") {
                this.celular = '';
            }
            else {
            ...
            this.datacadastro = data[0].datacadastro;
        },
        async validate() {
            const { valid } = await this.$refs.form.validate()

            if (valid == true && this.nome != null && this.situacao != null && this.email != null) this.update()
        ...
        async update() {
            ...
            await fetch(`${this.$baseUrlApiMain}/fornecedor/update/${this.$route.params.id}`, {
                method: "PUT",
