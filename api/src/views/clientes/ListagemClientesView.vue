        ...
        <v-row class="mt-4">
            ...
            <v-col cols="4" sm="12" md="4">
                <v-btn variant="tonal" rounded="lg" size="large" color="info" append-icon="mdi-account-plus-outline"
                    elevation="5" to="/clientes/cadastrarcliente">
                    Incluir Cliente
                </v-btn>
            </v-col>
        </v-row>
        ...
        <v-table fixed-header style="text-align:center">
            ...
            <tbody>
                <tr v-for="item in clientes" :key="item.idcliente">
                    <td :id="item.idcliente">{{ item.nome }}</td>
                    <td>{{ item.situacao }}</td>
                    <td>{{ item.tipocliente }}</td>
                    <td>{{ item.email }}</td>
                    <td>{{ item.tel }}</td>
                    <td>
                        <v-btn size="large" variant="tonal" @click="levarDados(item.idcliente)" icon color="info">
                            <v-icon>mdi-pencil-outline</v-icon>
                        </v-btn>
                        <v-btn size="large" variant="tonal" @click="deleteCliente(item.idcliente)"
...
export default {
    ...
    methods: {
        async getCliente() {
            ...
            const req = await fetch(`${this.$baseUrlApiMain}/cliente/read/${companyId}`,{
                method: "GET",
        ...
        async deleteCliente(id) {
            ...
            const req = await fetch(`${this.$baseUrlApiMain}/cliente/delete/${id}`, {
                method: "DELETE",
                headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`  }
            });
            const res = await req;
        ...
        async levarDados(id) {
            router.push(`/clientes/alterarcliente/${id}`)
        },
        async search() {
            const company = localStorage.getItem("company");
            let companyId = company.split("-");
